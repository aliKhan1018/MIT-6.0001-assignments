...
def is_valid_word(word, hand, word_list):
    ...
    new_hand = hand.copy()
    # if * is in the word. check against every vowels if it makes a word.
    if '*' in word:
        for char in VOWELS:
            word_copy = word.replace('*', char)
    ...
    for char in word:
        if not (char in hand) or new_hand[char] == 0:
            return False
...
def play_hand(hand, word_list):
    ...
    * The hand finishes when there are no more unused letters.
    ...
    while calculate_handlen(hand) > 0:
        ...
        display_hand(hand)
        # Ask user for input
        word = input("Enter a word or '!!' to indicate that you're finished: ")
...
def substitute_hand(hand, letter):
    ...
    new_letter_value = hand[letter]
    new_hand = hand.copy()
    
    while new_letter == letter and new_letter in hand.keys():
        new_letter = random.choice(ALPHABET)

    del new_hand[letter]
    new_hand[new_letter] = new_letter_value
...
def play_game(word_list):
    ...
    * Returns the total score for the series of hands
    ...
    current_no_of_hand = 1

    while current_no_of_hand <= no_of_hands:
        print(SEPERATOR)
        ...
        if replay == 'yes' and can_replay:
            hand = deal_hand(HAND_SIZE)
            score_replay = play_hand(hand, word_list)
            if score > score_replay:
                total_score += score
            else:
                total_score += score_replay
            can_replay = False 
        else:
            total_score += score
            current_no_of_hand += 1
